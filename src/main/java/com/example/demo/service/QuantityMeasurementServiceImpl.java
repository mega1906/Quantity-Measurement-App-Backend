...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
	...
	public double convert(QuantityDto q, Unit targetUnit) {
		try {
			...
			saveHistory(q, null, OperationType.CONVERT, result, false, null);
			return result;

		} catch (Exception ex) {
			// Save failed conversion with error flag and message.
			saveHistory(q, null, OperationType.CONVERT, null , true, ex.getMessage());
			throw ex;
		}
	}
	...
	public String compare(QuantityInputDto input) {
		...
		saveHistory(input.getThisQuantityDto(), input.getThatQuantityDto(), OperationType.COMPARE, resultString, false, null);
	 ...
	 */
	private void saveHistory(QuantityDto thisDto, QuantityDto thatDto, OperationType operation, Object result, boolean isError, String errorMessage) {
		...
		if (result instanceof Double) {
			...
			entity.setResultUnit(thisDto.getUnit().name());
			entity.setResultMeasurementType(thisDto.getUnit().getType().name());
	...
	public double arithmetic(ArithmeticRequestDto request) {
		...
		saveHistory( request.getThisQuantity(), request.getThatQuantity(),OperationType.valueOf(request.getOperation().name()),result, false,null);
